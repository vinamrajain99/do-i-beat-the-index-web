...
- **No claim is made anywhere in the repo that the math was inherited from another project** — this is intentional. The math is what it is; its lineage is private project history, not user-facing context. If contributors ever ask "where did this come from?", CLAUDE.md still links to the original CLI repo for internal reference.

---

## 2026-05-25 — Global top nav + `/about` page; trim now-redundant local auth chrome

**Context.** Up to today the app had no global navigation. Each page reached for its own affordances: `/auth/*` had an absolute "← Home" link in the layout, `/dashboard` had an inline "Signed in as `<email>`" + Sign-out form in its header, and nothing in the app explained the methodology — the README on GitHub was the only place. The user asked for both an About page (motivation, math, data sources) and a top-level menu bar to navigate between Home, About, and the analysis pages.

**Decision.**

- **Single global nav in the root layout** ([src/app/layout.tsx](src/app/layout.tsx)). Renders above `{children}` on every route, including `/auth/*`. Implemented as two files:
  - [src/components/nav-bar.tsx](src/components/nav-bar.tsx) — server component, calls `getUser()` and renders the signed-in vs signed-out variant.
  - [src/components/nav-link.tsx](src/components/nav-link.tsx) — small client component handling the active-route highlight via `usePathname()`. The bulk of the nav stays server-rendered.
- **New public `/about` route** at [src/app/about/page.tsx](src/app/about/page.tsx). Not in `PROTECTED_PREFIXES` (see [src/lib/supabase/proxy.ts](src/lib/supabase/proxy.ts)), so both signed-in and signed-out visitors get it. Content duplicates the methodology sections of `README.md` with light copyedits. Closing CTA conditional on `!user`.
- **Local affordances trimmed** now that the global nav owns brand-as-home, navigation links, email, and sign-out:
  - `/auth/*` layout no longer has a "← Home" link.
  - `/dashboard`'s header is just `<h1>Your analyses</h1>`. The "Signed in as `<email>`" subtitle and Sign-out form moved entirely to the nav.

**Alternatives considered.**

- **Per-page sub-navigations** (keep dashboard's local sign-out; add About link only to the landing page footer). Rejected: too easy to lose the About entry-point, and dual Sign-out buttons (page + nav) on `/dashboard` would have been silly once a global nav existed.
- **About content lives in a separate static `.md` rendered through a markdown component.** Would let the README and About diverge intentionally. Rejected for scope — pure JSX with Tailwind matches every other page in the project; avoids pulling in a markdown renderer for one page. Cost is the duplication burden flagged in Consequences and tracked in TODO.md → Later.
- **Make the nav a fully client component.** Would let one component own both the user fetch and the pathname. Rejected — `getUser()` naturally belongs server-side; the hybrid (server `NavBar` + small client `NavLink` children) matches the established pattern in this repo (cf. `analysis-runner.tsx`).
- **Hide the nav on `/auth/*` pages** to avoid the slight visual redundancy of a chrome bar above the centered auth card. Rejected — predictability is worth more than the small visual win, and a user on `/auth/login` can still hop to `/about` or click the brand to leave.

**Consequences.**

- **About-page copy is duplicated with README.md.** Same pattern as the 2026-05-24 "README is self-contained" ADR documents for the CLI ↔ web README split. Tracked in TODO.md → Later. Future changes to the math rationale need to land in both `README.md` and `src/app/about/page.tsx`.
- **Two `getUser()` calls per request** — once in the proxy (`updateSession`), once in the nav. Both are cheap (cached JWKS for the asymmetric flow; single Supabase REST call) and this is the standard SSR Supabase pattern. Not worth wrapping in a shared `cache()` helper unless profiling shows it.
